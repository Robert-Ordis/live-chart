public class Example : Gtk.Window {
    ...
    public Example() {
        ...
        chart.config.movable_timeline = true;
        ...
        var reticle = chart.new_point_reticle("test");
        //clicking legend->give the clicked serie to the reticle.
        chart.on_legend_clicked.connect((device, serie, btn_num) => {
            if(reticle.target_serie == serie){
                reticle.target_serie = null;
            }
            else{
                reticle.target_serie = serie;
            }
        });
        
        //move reticle to the pointer.
        chart.on_chart_motioned.connect((device, x, y) => {
            reticle.aim_to_point(x, y, true);
        });
        
        //click->fix the reticle's point or release
        chart.on_chart_clicked.connect((device, x, y, btn_num) => {
            reticle.locked = !(reticle.locked);
        });
        
        chart.on_chart_scrolled.connect((device, dx, dy) => {
            chart.config.time.current += (int)(500.0 * dy);
        });
